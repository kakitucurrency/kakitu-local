...
# Start genesis node with bash to keep it running
docker run -d --name kakitu_genesis \
  ...
  --user nanocurrency \
  kakitucurrency/kakitu-node:latest \
  /bin/bash -c "kakitu_node --daemon --network=dev --data_path=/home/nanocurrency/KakituDev & tail -f /dev/null"
...
# Start node 1 with bash to keep it running
...
# Start node 2 with bash to keep it running
